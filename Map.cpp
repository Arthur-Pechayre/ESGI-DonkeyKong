...
void Map::loadMap(const std::string& mapPath)
{
    ...
    while (std::getline(file, readedLine)) {
        std::vector<ABlock_shrdp> b;
        ++this->size.y;
        for (int x = 0; x < readedLine.size(); ++x) {
            ABlock_shrdp neoBlock = bg.CreateBlock(readedLine[x], *ressourcesManager, sf::Vector2f((float)((x + 1) * 32), (float)((this->size.y) * 32)));
    ...
    for (int y = 0; y < this->size.y; ++y) {
        if (y == 0 || y == this->size.y - 1) {
            ...
            for (int x = 0; x < this->size.x; ++x) {
                ABlock_shrdp neoBlock = bg.CreateBlock(0, *ressourcesManager, sf::Vector2f((float)(x * 32), (float)(y * 32)));
...
void Map::initPlayerOnBlock(ABlock_shrdp block)
{
    this->initPlayerPos = block->getPosition();
    ++this->initPlayerPos.y;
}

void Map::initDiamondOnBlock(ABlock_shrdp block)
{
    this->initDiamondsPos.push_back(sf::Vector2f(block->getPosition()));
}

void Map::registerSpawner(ABlock_shrdp block)
{
    this->spawners.push_back(std::dynamic_pointer_cast<PufferfishSpawnerBlock>(block));
}

void Map::initEntitiesPositions(const char& c, ABlock_shrdp block)
{
